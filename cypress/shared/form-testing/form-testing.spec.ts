	...
	public isFormWorking(link: string, submit: string, refreshForm: any) {
		describe('Общий тест формы', () => {
			it('Проверяем отправку без обязательных полей', () => {
				formTestingObject.submitWithoutRequiredFields(link, submit, refreshForm);
			});
		});
	}

	public isPopupFormWorking(link: string, submit: string, openFormButton: string, refreshForm: any) {
		describe('Попап', () => {
